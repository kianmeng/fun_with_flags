defmodule FunWithFlags.Store.Persistent.RedisTest do
  ...
  describe "put(flag_name, %Gate{})" do
    ...
    test "put() can change the value of a flag", %{name: name, gate: first_bool_gate} do
      ...
      assert {:ok, %Flag{name: ^name, gates: [^other_bool_gate, ^actor_gate]}} =
               PersiRedis.get(name)

      PersiRedis.put(name, first_bool_gate)

      assert {:ok, %Flag{name: ^name, gates: [^first_bool_gate, ^actor_gate]}} =
               PersiRedis.get(name)
    end
    ...
    test "put()'ing more gates will return an increasily updated flag", %{name: name, gate: gate} do
      ...
      assert {:ok, %Flag{name: ^name, gates: [^gate, ^other_gate]}} =
               PersiRedis.put(name, other_gate)
    end
    ...
    test "put() will UPSERT gates, inserting new ones and editing existing ones", %{
      ...
      assert {:ok, %Flag{name: ^name, gates: [^other_bool_gate, ^first_actor_gate]}} =
               PersiRedis.get(name)

      PersiRedis.put(name, first_bool_gate)

      assert {:ok, %Flag{name: ^name, gates: [^first_bool_gate, ^first_actor_gate]}} =
               PersiRedis.get(name)
      ...
      assert {:ok,
              %Flag{name: ^name, gates: [^first_bool_gate, ^first_actor_gate, ^other_actor_gate]}} =
               PersiRedis.get(name)
      ...
      assert {:ok,
              %Flag{
                name: ^name,
                gates: [^first_bool_gate, ^first_actor_gate_disabled, ^other_actor_gate]
              }} = PersiRedis.get(name)
      ...
      refute match?(
               {:ok,
                %Flag{
                  name: ^name,
                  gates: [^first_bool_gate, ^first_actor_gate, ^other_actor_gate]
                }},
               PersiRedis.get(name)
             )
  ...
  describe "delete(flag_name, %Gate{})" do
    ...
    test "delete(flag_name, gate) can change the value of a flag", %{
      ...
      assert {:ok, %Flag{name: ^name, gates: [^bool_gate, ^group_gate, ^actor_gate]}} =
               PersiRedis.get(name)
