defmodule FunWithFlags.Store.Persistent.EctoTest do
  ...
  describe "put(flag_name, %Gate{}), for boolean, actor and group gates" do
    ...
    test "put() will UPSERT gates, inserting new ones and editing existing ones", %{
      name: name,
      gate: first_bool_gate
    } do
      assert {:ok, %Flag{name: ^name, gates: []}} = PersiEcto.get(name)
      ...
      expected_flag =
        make_expected_flag(name, [other_actor_gate, first_actor_gate, first_bool_gate])
      ...
      expected_flag =
        make_expected_flag(name, [other_actor_gate, first_actor_gate_disabled, first_bool_gate])
      ...
      expected_flag =
        make_expected_flag(name, [
          other_actor_gate,
          first_actor_gate_disabled,
          first_bool_gate,
          first_group_gate
        ])
  ...
  describe "put(flag_name, %Gate{}), for percentage_of_time gates" do
    ...
    test "put() can change the value of a flag", %{name: name, pot_gate: pot_gate} do
      ...
      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^other_pot_gate]}} =
               PersiEcto.get(name)
    ...
    test "put()'ing more gates will return an increasily updated flag", %{
      name: name,
      pot_gate: pot_gate
    } do
      ...
      assert {:ok, %Flag{name: ^name, gates: [^bool_gate, ^pot_gate]}} =
               PersiEcto.put(name, pot_gate)
    end
  end
  ...
  describe "put(flag_name, %Gate{}), for percentage_of_actors gates" do
    ...
    test "put() can change the value of a flag", %{name: name, poa_gate: poa_gate} do
      ...
      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^other_poa_gate]}} =
               PersiEcto.get(name)
    ...
    test "put()'ing more gates will return an increasily updated flag", %{
      ...
      assert {:ok, %Flag{name: ^name, gates: [^bool_gate, ^poa_gate]}} =
               PersiEcto.put(name, poa_gate)
    end
  end
  ...
  describe "delete(flag_name, %Gate{}), for boolean, actor and group gates" do
    ...
    test "deleting()'ing the same gate multiple time is a no-op. In other words: deleting a gate is idempotent
         ...
         %{name: name, bool_gate: bool_gate, group_gate: group_gate, actor_gate: actor_gate} do
      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^bool_gate, ^group_gate]}} =
               PersiEcto.get(name)
  ...
  describe "delete(flag_name, %Gate{}), for percentage_of_time gates" do
    setup do
      ...
      flag = %Flag{
        name: name,
        gates: sort_gates_by_type([bool_gate, group_gate, actor_gate, pot_gate])
      }
    ...
    test "delete(flag_name, gate) can change the value of a flag",
      ...
      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^bool_gate, ^group_gate, ^pot_gate]}} =
               PersiEcto.get(name)

      PersiEcto.delete(name, group_gate)

      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^bool_gate, ^pot_gate]}} =
               PersiEcto.get(name)
    ...
    test "deleting()'ing more gates will return an increasily simpler flag",
      ...
      assert {:ok, %Flag{name: ^name, gates: [^bool_gate, ^group_gate, ^pot_gate]}} =
               PersiEcto.delete(name, actor_gate)
  ...
  describe "delete(flag_name, %Gate{}), for percentage_of_actors gates" do
    setup do
      ...
      flag = %Flag{
        name: name,
        gates: sort_gates_by_type([bool_gate, group_gate, actor_gate, poa_gate])
      }
    ...
    test "delete(flag_name, gate) can change the value of a flag",
      ...
      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^bool_gate, ^group_gate, ^poa_gate]}} =
               PersiEcto.get(name)

      PersiEcto.delete(name, group_gate)

      assert {:ok, %Flag{name: ^name, gates: [^actor_gate, ^bool_gate, ^poa_gate]}} =
               PersiEcto.get(name)
    ...
    test "deleting()'ing more gates will return an increasily simpler flag",
      ...
      assert {:ok, %Flag{name: ^name, gates: [^bool_gate, ^group_gate, ^poa_gate]}} =
               PersiEcto.delete(name, actor_gate)
