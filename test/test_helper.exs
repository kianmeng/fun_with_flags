...
IO.puts(
  "Erlang/OTP version:    #{:erlang.system_info(:system_version) |> to_string() |> String.trim_trailing()}"
)
...
IO.puts(
  "RDBMS driver:          #{inspect(if FunWithFlags.Config.persist_in_ecto?(), do: FunWithFlags.Dev.EctoRepo.__adapter__(), else: nil)}"
)
